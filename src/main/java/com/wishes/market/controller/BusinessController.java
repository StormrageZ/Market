...
import com.wishes.market.utils.RedisUtil;
...
public class BusinessController {
    ...
    public PageDto<CommodityVo> queryCommodities(CommodityQueryRequest request) {
        PageDto<CommodityVo> res;
        if(request.getCommodityType()!=null) {
            if (redisUtil.get(request.getCommodityType().toString()) == null) {
                res = businessService.queryCommodities(request);
                redisUtil.set(request.getCommodityType().toString(), res);
            } else res = (PageDto<CommodityVo>) redisUtil.get(request.getCommodityType().toString());
        }
        else res = businessService.queryCommodities(request);
        return res;
    }
